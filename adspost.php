		...
		<div class="col-sm-6 shadow-sm">
			...
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="file" name="pic" id="image" class="form-control-file border" accept=".mp4,.mkv,.webm" data-req>
				</div>
			</div>
			<br>
			<div class="row">
			...
			<div class="row">
				<div class="col-sm-10">
					<div class="input-group">
						<input type="number" name="rent" class="form-control" placeholder="Enter Rent Price" data-req>&nbsp;
						<input type="number" name="advance" class="form-control" placeholder="Enter Advance Price" data-req>
					</div>
				</div>
			</div>
			<br>
...
if (isset($_POST["adpost"])) {
    ...
    $sql="insert into adspost(room,kitchen,bath,area,rent,advance,address,image,document,owner,approve)
    values('$room','$kitchen','$bath','$area','$rent','$advance','$address','$f_new','$f_dnew','$owner','no')";
 ...
 */
 ...
 <script>
	...
	pic.addEventListener('change', function(e){
		...
		if(path[path.length-1]!=='mp4' && path[path.length-1]!=='mkv' && path[path.length-1]!=='webm'){
